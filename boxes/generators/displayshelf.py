...
class DisplayShelf(Boxes): # change class name here and below
    ...
    def generate_sides(self, width, height):
        if self.slope_top:
            ...
            if not self.include_back:
                edges = 'e'

            self.polygonWall(borders, edge=edges, callback=[self.generate_finger_holes], move="up", label="left")
            self.polygonWall(borders, edge=edges, callback=[self.generate_finger_holes], move="up", label='right')
        else:
            edges = "eeee"
